...
>
> **实现说明（2026-09-18 实施后补记）**：本方案最初设计为复用 `server.token`，实施评审时改为**面板独立口令** `ops.token`（理由：浏览应保持简单，且面板作为最高风险功能需要可独立吊销的凭据）。
> 因此下文 §5.2 L1、§6 配置样例等处出现的 `server.token` 应读作 `ops.token`；实际实现与测试以代码和 `README.md` 为准。
