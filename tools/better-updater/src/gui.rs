// Tier: 2
//! 原生 Win32 极简更新进度对话框（独立 UI 线程防假死、双模式动效、关闭按钮防护、零第三方依赖）。
...
impl GuiProgress {
    ...
    pub fn set_progress(&self, current: u64, total: u64, detail_text: &str) {
        if let (Some(tx), Some(hwnd)) = (&self.tx, self.hwnd) {
            let pct = if total == 0 {
                0
            } else {
...
fn run_gui_thread(title: String, init_tx: Sender<Option<isize>>, rx: Receiver<GuiMsg>) {
    unsafe {
        ...
        // Create fonts
        let font_face = wide("Microsoft YaHei UI");
        ...
        let progress_cls = wide("msctls_progress32");

        // Main status label (bold)
        let label_main = CreateWindowExW(
            0,
            static_cls.as_ptr(),
            wide(crate::strings::get().preparing).as_ptr(),
                ...
                while let Ok(gui_msg) = rx.try_recv() {
                    match gui_msg {
                        GuiMsg::SetStatus { main, marquee } => {
                            SetWindowTextW(label_main, wide(&main).as_ptr());
                        ...
                        GuiMsg::SetProgress { pct, detail } => {
                            ...
                            SetWindowTextW(label_detail, wide(&detail).as_ptr());
