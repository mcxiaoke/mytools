...
>
> **📌 决策（2026-09-15）：本期不启用签名。** 项目方决定首发**不**填入发布公钥，
> 即正式对外发布时签名边界仍为 0。该决策已作为**已接受偏差**记录，并附带以下补偿与义务：
> 1. `USAGE.md` §9 与 `RELEASE-NOTES.md` §1 必须**显式声明**"包真伪仅依赖 HTTPS 通道"，且 `--sha256` 只提供完整性、不可替代真实性；
> 2. 分发通道必须为 HTTPS，且优先放在带鉴权或不可篡改的托管上；
> 3. 一旦日后启用：补齐 `sig_missing_fail_closed` / `sig_tampered` / `sig_hex_and_bin` 三条用例，并走完本节的密钥轮换流程（**不得直接换钥**，否则所有旧客户端永久失联）。
> 该决策的复核意见（为什么它值得重新评估）见 `updater-vs-better-updater-reliability-recheck.md` §4 风险 2 与 §9。
