...
9. **集成契约交付物**：调用方文档**建议**包含"启动自检 → 同步执行 `--recover`"（**已由硬性契约降为可选加固**：该代码在"应用已被打断到起不来"时执行不到，故不能作为承诺前提；取而代之的是无参数自身修复入口与保留更新包，见 `USAGE.md` §12/§13），以及"回退触发点 / 防重装"约定；坏版本兜底首选 `--rollback-previous`，次选"重下旧包 + `--allow-downgrade`"；
