...
| **L3** | **冷启动自愈**：任意 updater 启动（含 `--recover`）优先介入。**其可靠性依赖调用方遵守 §2.3 的硬性集成契约**——否则 L2 被延后发布时，"确定性收敛"承诺不成立 | 看门狗亦被杀、系统断电、长期遗留 | 下次启动 |
...
**为什么是硬性契约**：本设计承诺"任意时刻被强杀/终止/掉电均可确定性收敛"，而 L3 的触发条件是"**任意 updater 启动**"——这由调用方决定。L2 看门狗又被列为**可延后发布**（`SCOPE.md` §2.2）。因此"L2 缺失 + 调用方不集成启动自检"这个组合会让承诺不成立：Worker 在 `APPLYING` 阶段被杀后，目录停在半更新状态，而下次被拉起的是**半新半旧的程序**。

**二者至少落实一项**：要么本契约不可选，要么 L2 不可延后。本设计选择前者，并把该契约作为交付物（`SCOPE.md` §2.3）与 DoD 项。
...
| 6 | **包内 EXE/DLL Authenticode**（可选，首发不做） | `--verify-authenticode` | 回滚 |
...
## 4. 版本回退 `--rollback-previous`（**首发暂缓实现**）
