...
### 2.3 集成契约：L3 的真实边界（2026-09-16 修订）

> 主程序每次启动时，若检测到 `<target>/.updater/journal` 存在，**建议**先同步执行
> `updater.exe --recover --target <dir>` 并等待其退出码，再继续自身启动。

**它为什么从"硬性"降为"建议"**：L3 的触发条件是"下次有 `updater.exe` 被启动"，而这由调用方决定。
更进一步——一次被中断的更新可能让**启动闭包**（入口 exe 及其 loader 期依赖、运行时数据）停在
半新半旧，此时应用**在 loader 阶段就失败**，`main()` 根本执行不到，启动自检代码也就跑不到。
**"能收敛"与"能被触发"是两件事**：把后者当成前者，就写出了一个不可执行的承诺。

因此本项目的承诺范围是 **L1 + L2**（进程内失败 / 施工进程被杀而看门狗存活），见 `USAGE.md` §8.3。
L2 覆盖的是**常见情况**（杀软清理、启动器清进程树、`taskkill`、用户手滑关窗口），不是极端情况；
真正落到 L3 的是掉电/重启这一档。

**取代"硬性契约"的两条路径**（都不依赖宿主能否启动）：

1. `updater.exe` **无参数**运行 = 收敛自身所在目录（`USAGE.md` §12）——让人能在安装目录里双击一下；
2. **保留更新包**，必要时解压覆盖（`USAGE.md` §13）——附带 `.updatekeep` 的前提条件。

**仍然建议做启动自检**：它让"崩在中途"的更早收敛，而不是等到下一次更新才处理。
只是它不再是承诺成立的前提。
