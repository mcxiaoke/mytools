...
**已开工（2026-09-14）。** Phase 0–6 已实现并通过测试：直接依赖 7、Tier 0 无并发原语、**114 项测试**
（默认 feature 96 项 + `--features pack` 18 项）全绿、release 产物 468.5 KB。**L2 看门狗与 `--rollback-previous` 回退引擎已落地**（2026-09-14 21:05 解除延后）；**承诺范围已于 2026-09-16 收缩为 L1+L2**，L3 为尽力而为，并补了两条不依赖宿主的路径（无参数自愈、保留更新包）。**崩溃窗口的真实进程终止验证已完成**（2026-09-15）。**Phase 7 Authenticode 搁置**（无代码签名证书，条件不具备）；**包签名本期不启用**（`unsigned-build`，见 `DESIGN.md` §6.1 的决策记录与 `RELEASE-READINESS.md` P0-1）。交付前剩余工作见 `RELEASE-READINESS.md`，最近变更见 `CHANGES-20260916.md`。Go 版 `updater.exe` 仍可作为行为对照基线。
...
| `PROPOSAL-pack-subcommand.md` | **打包能力 `packer`** 的形态决策（D1–D4）与实施计划；含三条实测约束（依赖 / 体积 / 跨平台） | — | 已决策并实施 |
